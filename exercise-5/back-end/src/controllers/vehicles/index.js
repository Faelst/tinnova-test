const {
  getJsonOnFile,
  registerNewVersionOfFileData,
} = require('../../helpers/get-json-data');
...
class Vehicles {
  ...
  updateVehicle(req, res) {
    ...
    const vehicleFinded = vehiclesJson
    ...
    for (const [i, e] of Object(vehiclesJson).entries()) {
      if (e.id == id) vehiclesJson.splice(i, 1, { ...e, ...vehicleUpdateData });
    }
  ...
  deleteVehicle(req, res) {
    ...
    for (const [i, e] of Object(vehiclesJson).entries()) {
      if (e.id == id) vehiclesJson.splice(i, 1, vehicleDeleted);
    }
