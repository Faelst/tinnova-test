...
export default function Home() {
  ...
  const [vehicleUpdateFormData, setVehicleUpdateFormData] =
  ...
  const handleChangeInputUpdateForm = ({ target }) =>
    setVehicleUpdateFormData({
      ...vehicleUpdateFormData,
      [`${target.name}`]: target.value,
    });
  const handleChangeCheckboxSold = ({ target }) =>
    setVehicleUpdateFormData({
      ...vehicleUpdateFormData,
      [`${target.name}`]: !vehicleUpdateFormData.vendido,
    });
  ...
  const saveNewVehicle = async () => {
    ...
    if (data.status == 201) setRegisterApiResponse(data.data);
    else
  ...
  const deleteVehicle = async () => {
    const data: any = await ApiService.deleteVehicle({
      vehicleId: vehicleUpdateFormData.id,
    });
            ...
            <tbody>
              {vehicleList.map((vehicle) => (
                <tr key={vehicle.id}>
                  ...
                  <td>{vehicle.marca}</td>
                  <td>
                    {String(vehicle.descricao).length > 200
                      ? `${vehicle.descricao.slice(0, 240)} ....`
                      : vehicle.descricao}
                  </td>
          ...
          <div>
            ...
            <div className={style.formGroup}>
              ...
              <textarea
                ...
                name="descricao"
                onChange={handleChangeInput}
                value={vehicleFormData.descricao}
              />
            </div>
            <div className={style.formBtnGroup}>
              ...
              <button type="button" onClick={saveNewVehicle}>
                {' '}
                Salvar
              </button>
            </div>
          ...
          <div>
            <div className={style.formGroup}>
              ...
              <input
                ...
                onChange={handleChangeInputUpdateForm}
                value={vehicleUpdateFormData.id}
              />
            </div>
            <div className={style.formGroup}>
              <label
                className={!enableUpdateFormData ? style.formLabelDisabled : ''}
              >
                Veiculo:
              </label>
              <input
                value={vehicleUpdateFormData.veiculo}
                type="text"
                name="veiculo"
                onChange={handleChangeInputUpdateForm}
                disabled={!enableUpdateFormData}
              />
            </div>
            <div className={style.formGroup}>
              <label
              ...
              <select
                ...
                name="marca"
                onChange={handleChangeInputUpdateForm}
                value={vehicleUpdateFormData.marca}
                disabled={!enableUpdateFormData}
              >
                <option value="">Selecione marca do veiculo</option>
                {vehicleBrandList.map((e: String | any) => (
            ...
            <div className={style.formGroup}>
              ...
              <input
                ...
                name="ano"
                onChange={handleChangeInputUpdateForm}
                value={vehicleUpdateFormData.ano}
                disabled={!enableUpdateFormData}
              />
            </div>
            <div className={style.formGroup}>
              ...
              <textarea
                ...
                name="descricao"
                onChange={handleChangeInputUpdateForm}
                value={vehicleUpdateFormData.descricao}
                disabled={!enableUpdateFormData}
              />
            </div>
            <div>
              <input
                ...
                id="vehicleSold"
                name="vendido"
                onClick={handleChangeCheckboxSold}
                checked={vehicleUpdateFormData.vendido}
              />
